...
The IBKR Excel RTD sample registers as `Tws.TwsRtdServerCtrl`. StreamXLS uses the shorter `Tws.Rtd` by default but can install a registry alias that points the legacy `Tws.TwsRtdServerCtrl` ProgID at its CLSID, so existing workbooks built against the sample keep working without edits. The alias is a registry entry only; it does not duplicate the binary. See [streamxls.com/docs](https://streamxls.com/docs) for the current procedure.
