...
`=RTD("tws.rtd", , "position", "{accounts}", "{contract}", "{field}")` (the field is named, never inferred; `{accounts}` may be blank for all accounts). Across multiple matched accounts, values aggregate (sizes and P&L sum; average cost is weighted by *absolute* size; contract-metadata fields return the matched contract when the spec resolves to a single contract ID, and fail loud with the same `Ambiguous position` error naming the matched contract IDs (or `Missing ConID` for a position TWS has not yet identified) when it resolves to more than one). A *partial* contract spec matters: in the aggregate (blank / `*` / CSV-accounts) form it aggregates across *every* contract it matches, whereas a single-account request does *not* aggregate — it must resolve to exactly one position. Adjusted and other non-standard-multiplier contracts roll into that same `Position` sum and `AverageCost` blend by raw contract count, even though `MarketValue` and the P&L fields already carry each contract's own multiplier; the tell is the `Multiplier` cell, which fails loud on the mismatch instead of picking one. A partial spec that matches several contracts in that account is ambiguous and fails loud (an `Ambiguous position` error naming the matched contract IDs, or `Missing ConID` for a position TWS has not yet identified) rather than returning an arbitrary one; the error clears on its own once the ambiguity resolves. Pin a single contract with enough contract detail, or `conid=`, for an unambiguous single-contract read.
...
| `REALIZEDPNL` | `RPNL` | Realized P&L (sums across accounts). Reads #N/A on a holding TWS has sent no realized figure for — most holdings, most of a session: TWS appears to report a realized figure for a position only once that position has realised something during the current session, so on a book you have not traded today the whole column reads #N/A. An aggregate is all-or-nothing about it: if any one matched position is missing the figure the cell reads #N/A rather than totalling the rest, so a multi-account or multi-contract roll-up reads #N/A unless *every* matched holding has one. (That all-or-nothing rule governs every position field that rolls up a number — `MARKETVALUE`, `DAILYPNL`, `UNREALIZEDPNL` and `AVERAGECOST` too; `REALIZEDPNL` is where you meet it.) |
...
- **One currency for that key** — the value. With a single currency there is nothing to aggregate, so that currency's figure is the account's. A currency sitting at exactly *zero* does not count as a second one (it adds nothing at any exchange rate), so an account that has zeroed out its other currencies keeps showing the value. Where TWS sends that one row *blank* — it does ship such rows on live accounts — the cell says so instead of showing nothing: `RTD error: TWS reported CRYPTOCURRENCY blank for USD on this account.` That is a TWS data condition rather than a formula to fix, so unlike the messages below it gives way to `#N/A` under `TWS_RTD_ERROR_DISPLAY=NA`, and it is replaced by the value as soon as TWS sends one. The same key asked for *with* that currency still reads empty: there the blank is the value TWS sent for the row you asked for.
