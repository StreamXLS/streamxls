...
Errors surface in a cell as text prefixed `RTD error:` (with a trailing space). For *market-data* subscription errors only, `TWS_RTD_ERROR_DISPLAY=NA` renders them as Excel `#N/A` instead (see [section 11](#11-configuration-keys)); the rest of the messages below are always text. A cell also shows Excel `#N/A` (COM error 2042) when data is unavailable or has been failed loud. Delayed values, when annotation is on, render as '150.25 (delayed)'. Notable messages:
